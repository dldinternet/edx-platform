...
def location_to_query(location, wildcard=True, tag='i4x'):
    ...
    rather than subdoc. Note: LocationBase.to_son is faster in mongo as it does subdoc equivalence.
    ...
    """
    query = location.to_son(prefix='_id.', tag=tag)
...
class MongoModuleStore(ModuleStoreWriteBase):
    ...
    def _query_children_for_cache_children(self, course_key, items):
        ...
        query = {
            '_id': {'$in': [
                course_key.make_usage_key_from_deprecated_string(item).to_son() for item in items
            ]}
        }
    ...
    def delete_item(self, location, **kwargs):
        ...
        # Must include this to avoid the django debug toolbar (which defines the deprecated "safe=False")
        # from overriding our default value set in the init method.
        self.collection.remove({'_id': location.to_son()}, safe=self.collection.safe)
        # recompute (and update) the metadata inheritance tree which is cached
